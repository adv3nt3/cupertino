...
### Added
- **Transitive dependency resolution for `fetch --type package-docs`** (#184): each seed in `priority-packages.json` is now walked through its `Package.resolved` (fetched from `raw.githubusercontent.com`) and its GitHub-hosted dependencies are added to the fetch queue. Non-GitHub URLs, missing manifests, and malformed manifests are counted and reported, then skipped. Opt out with `--no-recurse`. Terminates via URL dedupe, so a seed like `vapor` naturally pulls in `swift-nio`, `async-http-client`, `swift-log`, etc. without manual curation.
